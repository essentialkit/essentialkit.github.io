...
Url: ${_getEventFilterUrl(event)}`
  ...
  var SettingsUI = class extends HTMLElement {
    constructor(configItems) {
      super();
      this.template = new DOMParser().parseFromString(settings_txt_default, "text/html");
    ...
    async fetchAndSetConfigValues(options) {
      for (const option of options) {
        const val = await storage_default.get(option.id);
    ...
    async saveChange(config, updatedValue) {
      this.logger.debug("saving: ", config.id, updatedValue);
      await storage_default.put(config.id, updatedValue);
      this.showToast();
    }
    cloneInput(config) {
      let control = this.template.getElementsByClassName(`${config.type}-template`)[0].cloneNode(true);
      ...
      if (["checkbox", "switch"].includes(config.type)) {
        ...
        actualInput.addEventListener(
          "input",
          (e) => this.saveChange(config, e.target.checked)
        );
      } else {
      ...
      if (config.type === "range") {
        ...
        actualInput.addEventListener("input", (e) => {
          this.saveChange(config, e.target.value);
          actualInput.nextElementSibling.innerHTML = actualInput.value;
        });
      }
      if (config.type === "select") {
        ...
        actualInput.selectedIndex = config.options?.findIndex((o) => o.id === config.value) ?? -1;
        actualInput.addEventListener(
          "change",
          (e) => this.saveChange(config, e.target.value)
        );
      }
    ...
    showToast() {
      ...
      if (!toastEl) {
        ...
        toastEl.querySelector(".toast-body span").innerHTML = i18n("optionsSaveSuccess");
...
`;
  ...
  var Options = class {
    ...
    renderSettingsUI() {
      document.querySelector(".options-container")?.appendChild(new SettingsUI(configOptions));
    }
    setI18nText() {
      document.querySelector(".title").textContent = appName2;
      ...
      document.querySelector(".report-issue").textContent = i18n("reportAnIssue");
    }
